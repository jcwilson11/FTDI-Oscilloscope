...
The professor-facing adapter \texttt{ioFtdiDevice} is defined in \texttt{ftd2xx\_wrapper.py}. It preserves a simple device-oriented API while reusing the canonical session layer.
...
\subsection{Branch Context}
The earlier \texttt{origin/oscilloscope} branch provided the historical UI scaffold for this assignment. The current \texttt{dev} branch is the canonical architecture path, and the architecture document now reflects the implemented \texttt{io...} class structure rather than the earlier scaffold names.
...
\begin{itemize}
    ...
    Responsibility: Runs the professor-facing oscilloscope shell and persists runtime settings.
...
\begin{itemize}
    ...
    Responsibility: Provides the canonical professor-facing FTDI device adapter over the session layer.
...
\begin{itemize}
    \item \texttt{docs/plantuml/io\_scope\_logical\_class.puml} \\
    Purpose: Canonical logical class view showing the implemented classes and their relationships.

    \item \texttt{docs/plantuml/io\_scope\_logical\_class\_readable.puml} \\
    Purpose: Readable logical class view with short human-language responsibilities for grading and presentation.

    \item \texttt{docs/plantuml/io\_scope\_process.puml} \\
    Purpose: Process view showing the FTDI read thread, shared circular buffer, write thread, and live-session interaction.

    \item \texttt{docs/plantuml/io\_scope\_development.puml} \\
    Purpose: Development view showing the package and file organization of the current codebase.

    \item \texttt{docs/plantuml/io\_scope\_physical.puml} \\
    Purpose: Physical view showing the runtime environment, FTDI DLL dependency, and hardware deployment context.

    \item \texttt{docs/plantuml/io\_scope\_scenario\_sequence.puml} \\
    Purpose: Scenario view showing the user flow from startup through selection, processing, and rendering.
\end{itemize}

Supporting diagrams are also included in \texttt{docs/plantuml/}, including startup, controls, package, runtime-component, deployment, object, and multithreaded pipeline views.
...
The canonical class diagram for this assignment is the logical class view in \texttt{docs/plantuml/io\_scope\_logical\_class.puml}. The readable grading version is \texttt{docs/plantuml/io\_scope\_logical\_class\_readable.puml}.

\begin{figure}[H]
    \centering
    \IfFileExists{plantuml/io_scope_logical_class_readable.png}{
        \includegraphics[height=0.8\textheight]{plantuml/io_scope_logical_class_readable.png}
    }{
        \fbox{
            \parbox{0.82\textwidth}{
                \textbf{Class diagram asset note.} The submitted source-of-truth class diagrams are the PlantUML files
                \texttt{docs/plantuml/io\_scope\_logical\_class.puml} and
                \texttt{docs/plantuml/io\_scope\_logical\_class\_readable.puml}. If a rendered PNG is not present in the build
                directory, those PlantUML sources remain the authoritative architecture artifacts for the submission.
            }
        }
    }
    \caption{Class diagram for the current oscilloscope system architecture}
...
\begin{itemize}
    ...
    \item Demonstrating headless Qt startup through \texttt{python controller.py scope-qt --headless}.
\end{itemize}

\section{Documentation Update \\
\small{\textit{-- Joris Wilson, Eva Kemp, and Nica Saoi}}}
\index{Section!Documentation Update}
\label{Section::Documentation Update}

The documentation was updated to match the current codebase and the current diagram set. The revised narrative now uses the implemented \texttt{io...} class names, identifies \texttt{ioOscilloscopeView} as the single abstract View boundary, documents the compact and detailed Qt views as the two selectable user interfaces, explains the live-session integration, and references the PlantUML 4+1 artifacts that correspond to the implemented design.
