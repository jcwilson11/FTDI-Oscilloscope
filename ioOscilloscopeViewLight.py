"""Light-theme Qt oscilloscope View used by the assignment UI."""
...
class ioOscilloscopeViewLight(ioViewBase, QtWidgets.QWidget):
    """Light Qt View that renders waveform data without backend responsibilities.
    ...
    def __init__(self, parent: QtWidgets.QWidget | None = None):
        ...
        self._curve = self._plotWidget.plot(
            pen=pg.mkPen(color="#0f62fe", width=2),
            antialias=True,
        )
    ...
    def _configurePlot(self) -> None:
        self.setObjectName("ioOscilloscopeViewLight")
        self.setStyleSheet(
            """
            QWidget#ioOscilloscopeViewLight {
                background-color: #f8fafc;
            }
            """
        )

        self._plotWidget.setBackground("#f8fafc")
        self._plotWidget.showGrid(x=True, y=True, alpha=0.3)
        self._plotWidget.setMenuEnabled(False)
        self._plotWidget.setMouseEnabled(x=False, y=False)
        self._plotWidget.hideButtons()
        self._plotWidget.setLabel("left", "Amplitude", color="#475569")
        self._plotWidget.setLabel("bottom", "Samples", color="#475569")

        plotItem = self._plotWidget.getPlotItem()
        if plotItem is not None:
            plotItem.getAxis("left").setPen(pg.mkPen("#cbd5e1"))
            plotItem.getAxis("bottom").setPen(pg.mkPen("#cbd5e1"))
            plotItem.getAxis("left").setTextPen(pg.mkPen("#475569"))
            plotItem.getAxis("bottom").setTextPen(pg.mkPen("#475569"))
